...
#include "llvm/Support/raw_ostream.h"//SNEISIUS
...
void DwarfDebug::endModule() {
  ...
  //SNEISIUS
  //DIE labelDIE = DIE(dwarf::DW_TAG_map_label);
  //DIEInteger labelLoc = DIEInteger(777);
  //DIEString labelVal = DIEString(&labelLoc, "TEST");

  //errs() << "endModule: \n";
  //labelVal.dump();
  //labelDIE.addValue(dwarf::DW_AT_map_label, dwarf::DW_FORM_string, &labelVal);
  //labelDIE.dump();
  //errs() << "\n";
  //SNEISIUS END
...
void DwarfDebug::beginInstruction(const MachineInstr *MI) {
  ...
  if (MI->getMapLabel().size() != 0){//SNEISIUS
    //errs() << "\t\tDWARF:beginInstruction: " << MI->getMapLabel() << "\n";//SNEISIUS
    StringRef mapLabel = StringRef(MI->getMapLabel());
    Asm->OutStreamer.EmitDebugLabel(MMI->getContext().GetOrCreateSymbol(mapLabel) );
  }
