...
public:
  ...
  void EmitNode(SDNode *Node, bool IsClone, bool IsCloned,
                DenseMap<SDValue, unsigned> &VRBaseMap) {
    if (Node->getMapLabel().size() != 0){
      //errs() << "\tIE:EmitNode: " << Node->getMapLabel() << "\n";//SNEISIUS
      //MBB->dump();
      //errs()<<"\n";
      unsigned num=0;
      for (MachineBasicBlock::instr_iterator instr=MBB->instr_begin(), end=MBB->instr_end(); instr!=end ;instr++ ){
        //errs() << "!!!!";
        //instr->dump();
        std::ostringstream nStr;
        nStr << Node->getMapLabel() << ";" << num++;
        instr->setMapLabel(nStr.str());
      }
    }
    if (Node->isMachineOpcode()){
      EmitMachineNode(Node, IsClone, IsCloned, VRBaseMap);
    }else{
      //errs() << "EmitNode:"<< Node->isMachineOpcode() << ":"<< Node->getMapLabel() << "\n";//SNEISIUS
      EmitSpecialNode(Node, IsClone, IsCloned, VRBaseMap);
    } 
    //MBB->back().setMapLabel(Node->getMapLabel());
    //MBB->back().dump();
    //if(InsertPos!=NULL){InsertPos->dump();}
  }
