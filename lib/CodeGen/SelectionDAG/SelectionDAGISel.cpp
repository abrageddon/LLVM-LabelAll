  ...
  for (ReversePostOrderTraversal<const Function*>::rpo_iterator
    ...
    if (FastIS) {
      ...
      for (; BI != Begin; --BI) {
        ...
        if(Inst->getMetadata("label.all")){
          //errs() << "SD-ISel: " <<cast<MDString>(Inst->getMetadata("label.all")->getOperand(0))->getString() << "\n";
          MachineInstrBuilder::setCurrentMapLabel(cast<MDString>(Inst->getMetadata("label.all")->getOperand(0))->getString());//SNEISIUS
        }
